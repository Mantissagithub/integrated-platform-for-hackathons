...
import { Moon, Sun, ChevronRight, User, Mail, Lock, Briefcase } from 'lucide-react'
...
const spiderVerseColors = {
  ...
  dark: {
    ...
    text: '#FFFFFF',
    accent: '#FFEA00',
    formBg: 'rgba(0, 0, 0, 0.3)',
  },
}

const SpiderVerseSignup: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* <p className="text-xl sm:text-2xl font-comic" style={{ color: colors.secondary }}>Where web-slingers unite!</p> */}
      </motion.div>
      ...
      <div className="w-1/2 h-full flex flex-col items-center justify-center p-4 z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { src: 'https://media.istockphoto.com/id/1318360204/photo/team-meeting-and-brainstorming-in-modern-office.jpg?s=2048x2048&w=is&k=20&c=SkzLULKbTDf6U5jVN6DnjtnkYyV_jalx3hH5kg8yJ8E=', alt: 'Team web-slinging', icon: Briefcase },
            { src: 'https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?q=80&w=2047&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D', alt: 'Spidey coding', icon: Lock },
            { src: 'https://plus.unsplash.com/premium_photo-1661775083116-3e8fbb1d5a7b?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D', alt: 'Multi-verse design', icon: Mail },
            { src: 'https://media.istockphoto.com/id/1333390966/photo/diverse-employees-on-online-conference-video-call-on-tv-screen-in-meeting-room.jpg?s=2048x2048&w=is&k=20&c=zOKo0bgrIYhxO4KQouyhslGItFB9Cx8UxDiY_8_0MfQ=', alt: 'Spider-presentation', icon: User },
      ...
      <div ref={formRef} className="w-1/2 h-full flex items-center justify-center z-10">
        <div className="w-3/4 max-w-md p-8 rounded-xl shadow-2xl backdrop-blur-md relative overflow-hidden transition-all duration-500" style={{ backgroundColor: colors.formBg }}>
          ...
          <h2 className="text-3xl font-bold mb-6 relative z-10 font-comic" style={{ color: colors.accent }}>Create Your Account</h2>
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4 relative z-10">
            <AnimatePresence>
              {[
                { name: 'name', icon: User, placeholder: 'Your Spider-Name' },
                { name: 'email', icon: Mail, placeholder: 'your@spider.verse' },
                { name: 'password', icon: Lock, placeholder: 'Secret Web-Shooter Code' },
                { name: 'confirm-password', icon: Lock, placeholder: 'Confirm Web-Shooter Code' },
            ...
            >
              <span className="relative z-10">Swing into Action!</span>
          ...
          >
            <a href="/login" className="font-medium hover:underline font-comic" style={{ color: colors.secondary }}>
              Already in the HackDash? Swing in!
            </a>
          </motion.div>
        </div>
      </div>
      ...
      <div className={`absolute bottom-4 right-4 p-2 transform rotate-3 shadow-lg z-20 transition-colors duration-500 ${darkMode ? 'bg-yellow-400 text-black' : 'bg-red-500 text-white'}`}>
        <p className="font-bold font-comic text-sm">THWIP! Sign up for a spectacular adventure!</p>
      </div>
...
export default SpiderVerseSignup